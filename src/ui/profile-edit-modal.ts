...
export function openProfileEditModal(profilePath: string, customDisplay: string, group: string): void {
  ...
  hint.textContent = profilePath ? `${t("modal.edit.filenamePrefix")}${basename(profilePath)}` : "";
...
async function saveProfileEdits(originalPath: string): Promise<void> {
  ...
  if (!newPath) {
    setFeedback(t("feedback.emptyPath"), true);
    return;
  }
...
export function bindProfileEditModal(): void {
  ...
  pathInput.addEventListener("input", () => {
    ...
    hint.textContent = p ? `${t("modal.edit.filenamePrefix")}${basename(p)}` : "";
  });
  ...
  openEditor.addEventListener("click", () => {
    ...
    if (!target) {
      setFeedback(t("feedback.noPathToOpen"), true);
      return;
    }
